...
class AnimationStageList
{
        ...
        foreach (var v in tileSheet.Properties)
        {
            ...
            if (s1.Length > 1)
            {
                if (s1[1] == "frames")
                {
                    ...
                    var animationStage = new AnimationStage(nBeginFrame, nEndFrame, strName);
    ...
    public string AnimationStageName
    {
        ...
        set
        {
            var animationStage = GetAnimationStageByName(value);
...
abstract class GameObject
{
    ...
    public abstract void Draw(SpriteBatch spriteBatch, xTile.Dimensions.Rectangle viewPort);
    ...
    private void LoadSounds()
    {
        foreach (var v in m_tileSheet.Properties)
        {
            ...
            if (s1.Length > 1)
            {
                if (s1[1] == "sound")
                {
                    var strSoundName = s1[0];
                    var strSoundFileName = v.Value.ToString();
...
class ActorGameObject : GameObject
{
    ...
    private void CalcOnGround()
    {
        ...
        var fCircleRadius = ConvertUnits.ToSimUnits(m_fCircleRadius - 1);
        var pos = m_bodyCircle.Position;
        var spList = new Vector2[]
        { 
            new Vector2(pos.X - fCircleRadius, pos.Y),
            new Vector2(pos.X, pos.Y),
            new Vector2(pos.X + fCircleRadius, pos.Y)
        };
        var epAddition = ConvertUnits.ToSimUnits(0, 5 + m_fCircleRadius);
    ...
    private void CalcPlatformEdge()
    {
        ...
        var p = m_bodyCircle.Position;
        var p1 = ConvertUnits.ToSimUnits(m_fCircleRadius / 2 * ((m_bDirection) ? 1 : -1), m_fCircleRadius + 10);
    ...
    {
        var screenPos = Position - new Vector2(viewPort.X, viewPort.Y);
        var originPos = new Vector2(m_tileSheet.TileWidth / 2, m_tileSheet.TileHeight / 2);
        var r = m_tileSheet.GetTileImageBounds(m_animationStageList.AnimationStage.CurFrame);
        var sourceRectangle = new Rectangle(r.X, r.Y, r.Width, r.Height);
        var spriteEffects = (m_bDirection) 
            ? SpriteEffects.None
...
class GemGameObject : GameObject
{
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        for (var edge = m_bodyRect.ContactList; edge != null; edge = edge.Next)
        {
    ...
    {
        var p1 = m_bodyRect.Position;
        var p2 = p1 + ConvertUnits.ToSimUnits(0, m_fRectangleHeight);
...
class BulletGameObject : GameObject
{
        ...
        : base(world, tileSheet, texture)
    {
        ...
        var imp = new Vector2(0.008f * ((playerGameObject.Direction) ? 1f : -1f), -0.001f);
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        // Попадание в врага   
        for (var edge = m_body.ContactList; edge != null; edge = edge.Next)
        {
            if (edge.Contact.FixtureA.Body.Tag is EnemyGameObject)
            {
                var enemyGameObject = edge.Contact.FixtureA.Body.Tag as EnemyGameObject;
...
class EnemyGameObject : ActorGameObject
{
    ...
    public void Blow()
    {
        // Начинаем отсчет до анимации растворени
        m_bBlowPending = true;
