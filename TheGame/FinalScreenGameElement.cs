...
class FinalScreenGameElement : DrawableGameComponent
{
    ...
    public override void Draw(GameTime gameTime)
    {
        ...
        foreach (var emitter in m_exposionEmitterList)
    ...
    public override void Update(Microsoft.Xna.Framework.GameTime gameTime)
    {
        ...
        if (m_nExposionEmitterRemainMsec <= 0)
        {
            ...
            {
                var pos = new Vector2((float)Game.GraphicsDevice.Viewport.Width * (float)Random.NextDouble(),
                                           (float)Game.GraphicsDevice.Viewport.Height * (float)Random.NextDouble());
                var gemGameObject = m_panelManager.GemList[Random.Next(m_panelManager.GemList.Count)];
                var particleEffect = ParticleEffect2DFactory.Initialize(50, 2000)
